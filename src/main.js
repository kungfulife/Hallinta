  ...
  try {
    if (window.__TAURI__ && window.__TAURI__.core) {
      try {
        ...
        const defaultPath = 'C:\\Users\\facky\\AppData\\LocalLow\\Nolla_Games_Noita\\save00';
        document.getElementById('noita-dir').value = defaultPath;
        await loadModConfigFromDirectory(defaultPath);
      }
    } else {
      document.getElementById('noita-dir').value = 'C:\\Users\\facky\\AppData\\LocalLow\\Nolla_Games_Noita\\save00';
    }
...
window.exportModList = async function() {
  try {
    if (window.__TAURI__ && window.__TAURI__.core) {
      const exportData = await window.__TAURI__.core.invoke('export_mod_list', {
        mods: currentMods,
      ...
      a.href = url;
      a.download = `${selectedPreset}_mods.json`;
      a.click();
